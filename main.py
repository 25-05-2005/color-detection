...
# Check if the color dataset exists, if not, create it
if not os.path.exists(COLOR_CSV):
    ...
    colors_df = pd.read_csv(COLOR_CSV)  # Load existing dataset
...
if uploaded_image:
    ...
    # Select Pixel (Interactive)
    x = st.slider("Select X-coordinate:", 0, img_np.shape[1]-1, img_np.shape[1]//2)
    y = st.slider("Select Y-coordinate:", 0, img_np.shape[0]-1, img_np.shape[0]//2)
    ...
    # Display Detected Color Box ✅
    st.markdown(
        f'<div style="width:50px;height:50px;background-color:rgb({pixel_color[0]},{pixel_color[1]},{pixel_color[2]})"></div>',
